...
<?php
					if(isset($_POST["product_type"]))
						{
							...
							{
							if(strtolower($row['product_type_name']) == $product_type)
							{
								?>
					<script>
					...
					<?php
						...
						if($checking==0)
							{
							$check=mysqli_query($connect,"UPDATE product_type SET product_type_name='$product_type' WHERE product_type_id='$pid'");
								}
						if($check)
						{
?>
				...
				<?php
				header("refresh:0.2; url=superadmin_all-product-type.php");
					}
				}
				?>
		</div>
			...
			<div class="content container-fluid">
				<div class="page-header">
					<div class="row align-items-center">
						<div class="col">
							<h3 class="page-title mt-5">Edit Product Type</h3> </div>
					</div>
				</div>
				<div class="row">
					<div class="col-lg-12">
						...
						{
							$pid =$_GET['id'];
							$result = mysqli_query($connect, "SELECT *FROM product_type WHERE product_type_id='$pid'");
				 	...
				 	?>
							...
							<div class="col-md-4">
									<div class="form-group">
										...
										<input class="form-control" type="text" name="product_type_id" value="<?php echo $row['product_type_id'] ?>" readonly></div>
				...
				<a href="superadmin_all-product-type.php" class="btn btn-secondary">Back</a>
						</form>
						<?php
						}
						?>
